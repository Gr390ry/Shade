...
public:
	...
	static bool IsExactClass(const T2* pObj) { return pObj != NULL ? pObj->IsExactClass(&T1::s_RTTI) : false; }
	template<typename T>
	static bool IsExactClass(const RTTI* pRTTI, const T* pObj) { return (pRTTI != NULL && pObj != NULL) ? pObj->IsExactClass(pRTTI) : false; }
	template<typename T1, typename T2>
	static bool IsKindOf(const T2* pObj) { return pObj != NULL ? pObj->IsKindOf(&T1::s_RTTI) : false; }
	template<typename T>
	static bool IsKindOf(const RTTI* pRTTI, const T* pObj) { return (pRTTI != NULL && pObj != NULL) ? pObj->IsKindOf(pRTTI) : false; }
	template<typename T1, typename T2>
	static T1* DynamicCast(T2* pObj) { return pObj != NULL ? (pObj->IsKindOf(&T1::s_RTTI) ? static_cast<T1*>(pObj) : NULL) : NULL; }
	template<typename T1, typename T2>
	static const T1* DynamicCast(const T2* pObj) { return pObj != NULL ? (pObj->IsKindOf(&T1::s_RTTI) ? static_cast<const T1*>(pObj) : NULL) : NULL; }
	template<typename T>
	static unsigned int GetTypeID() { return reinterpret_cast<unsigned int>(&T::s_RTTI); }
	template<typename T>
	static unsigned int GetTypeID(const T* pObj) { return pObj != NULL ? reinterpret_cast<unsigned int>(pObj->GetRTTI()) : 0x00000000; }
	template<typename T>
	static const RTTI* GetRTTI() { return &T::s_RTTI; }
	template<typename T>
	static const RTTI* GetRTTI(const T* pObj) { return pObj != NULL ? pObj->GetRTTI() : NULL; }
